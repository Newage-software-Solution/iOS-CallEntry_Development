...
open class LineChart: UIView {
    ...
    */
    ...
    public struct Axis {
        public var visible: Bool = true
        // #607d8b
        public var color: UIColor = UIColor(red: 0.44, green: 0.44, blue: 0.44, alpha: 1)
    ...
    open var colors: [UIColor] = [
        UIColor(red: 0.97, green: 0.35, blue: 0.35, alpha: 1),
        UIColor(red: 0.11, green: 0.75, blue: 0.18, alpha: 1),
    ...
    fileprivate func drawDataDots(_ lineIndex: Int) {
        ...
        for index in 0..<data.count {
            let xValue = self.x.scale(CGFloat(index)) + x.axis.inset - dots.outerRadius/2 + extra
            let yValue = self.bounds.height - self.y.scale(data[index]) - y.axis.inset - dots.outerRadius/2
            
            // draw custom layer with another layer in the center
            let dotLayer = DotCALayer()
            dotLayer.dotInnerColor = lineIndex == 1 ? colors[lineIndex] : UIColor.white
            dotLayer.innerRadius = dots.innerRadius
            dotLayer.backgroundColor = colors[lineIndex].cgColor
    ...
    fileprivate func drawXLabels() {
        ...
        for (index, _) in xAxisData.enumerated() {
            let xValue = self.x.scale(CGFloat(index)) + x.axis.inset - (width / 2) + 5
            let label = UILabel(frame: CGRect(x: xValue, y: y, width: width, height: x.axis.inset))
            label.font = UIFont.init(name: "Roboto", size: 12.0)
            label.textColor = UIColor(red: 0.24, green: 0.24, blue: 0.24, alpha: 0.7)
