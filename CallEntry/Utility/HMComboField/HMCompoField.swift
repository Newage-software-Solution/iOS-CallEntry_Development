...
//            else
//            {
                self.pickerDelegate?.pickerSelected(index: 0, selectedTag : selectedTag, withData: self.text! as String as AnyObject)
            
            AppCacheManager.sharedInstance().followUpData = self.text! as String //updatefollowup // followUpDate
//            }
        ...
        else
        {
            ...
            if key != nil
            {
                ...
                else if multiLineSupport
                {
                    ...
                    self.text = ""
                    pickerDelegate?.pickerSelected(index: selectedIndex, selectedTag : selectedTag, withData: (list[selectedIndex] as AnyObject).value(forKey: self.key!) as! NSObject)
                    self.resignFirstResponder()
                    return
            ...
            pickerDelegate?.pickerSelected(index: selectedIndex, selectedTag : selectedTag, withData: list[selectedIndex] as AnyObject)
...
extension HMCompoField: UITextFieldDelegate {
    ...
    func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        ...
        if textField.text!.count + string.count > self.maxLength {
            return false
        }
